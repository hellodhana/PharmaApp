...
#import "DurabilityTableCell.h"
#import "PerformanceView.h"
#import "PercentageProgressBar.h"
#import "MBRoundProgressView.h"

@implementation DurabilityTableCell
@synthesize brand=_brand;
@synthesize performanceBarTarget=_performanceBarTarget;
@synthesize performanceBarCurrent=_performanceBarCurrent;
...
- (void)loadDataFromEntity:(id)entity 
{
   ...
   // DODurability *bo = (DODurability *)entity;
  /*  self.brand.text = bo.doBrand.brandDesc;
    ...
    //Performance current
    PercentageProgressBar *accessBarCurrent = [[PercentageProgressBar alloc] initWithFrame:CGRectMake(0, 0, self.performanceBarCurrent.frame.size.width, self.performanceBarCurrent.frame.size.height) andProgressBarStyle:ProgressStyle];
    [self.performanceBarCurrent addSubview:accessBarCurrent];
    
    float performance1 = [bo.salesAct doubleValue]/ [bo.salesPln doubleValue];
    [accessBarCurrent setProgress:performance1];    
    
    //Performance target
    PercentageProgressBar *accessBarTarget = [[PercentageProgressBar alloc] initWithFrame:CGRectMake(0, 0, self.performanceBarTarget.frame.size.width, self.performanceBarTarget.frame.size.height) andProgressBarStyle:ProgressStyle];
    [self.performanceBarTarget addSubview:accessBarTarget];
    
    float performance = [bo.salesAct doubleValue]/ [bo.salesPln doubleValue];
    [accessBarTarget setProgress:performance];
